...
class LimitModal(Modal, title="User-Limit festlegen"):
    limit = TextInput(label="Neues Limit (0–99)", max_length=2)
...
class RenameModal(Modal, title="Voice-Channel umbenennen"):
    name = TextInput(
        label="Neuer Name",
        max_length=32,
        placeholder="z.B. Chill Room"
    )
    ...
    async def on_submit(self, interaction):
        ...
        await interaction.response.send_message(
            f"✏️ Channel umbenannt zu **{self.name.value}**",
            ephemeral=True
        )
...
class DisconnectSelect(Select):
    ...
    async def callback(self, interaction):
        ...
        await interaction.response.send_message("🔌 User getrennt.", ephemeral=True)
...
class DragSelect(UserSelect):
    def __init__(self, channel):
        super().__init__(placeholder="User suchen & ziehen")
        self.channel = channel
    ...
    async def callback(self, interaction):
        ...
        if user.voice:
            try:
                ...
                pass
        await interaction.response.send_message("➡️ Aktion ausgeführt.", ephemeral=True)
...
class BlockSelect(UserSelect):
    def __init__(self, channel):
        super().__init__(placeholder="User blockieren")
        self.channel = channel
...
class TempVoice(commands.Cog):
    ...
    async def setup_tempvoice(self, ctx):
        if ctx.guild.id != ALLOWED_GUILD_ID:
            return await ctx.send("❌ Nicht erlaubter Server.")
        ...
        embed = discord.Embed(
            ...
            description=(
                "Steuere deinen **VC** mit den Buttons unten:\n\n"
        ...
        await ctx.send("✅ TempVoice eingerichtet.")
...
class VoiceChannelView(View):
    ...
    async def no_channel(self, interaction):
        await interaction.response.send_message(
            "❌ Du besitzt keinen privaten Voice-Channel.",
            ephemeral=True
        )
    ...
    async def claim(self, interaction, _):
        ...
        if interaction.user.voice:
            ...
            if owner not in [m.id for m in ch.members]:
                ...
                return await interaction.response.send_message("👑 Channel übernommen.", ephemeral=True)
        await interaction.response.send_message("❌ Channel nicht claimbar.", ephemeral=True)
    ...
    async def lock(self, interaction, _):
        ...
        await interaction.response.send_message("🔒 Gesperrt.", ephemeral=True)
    ...
    async def unlock(self, interaction, _):
        ...
        await interaction.response.send_message("🔓 Entsperrt.", ephemeral=True)
    ...
    async def ghost(self, interaction, _):
        ...
        await interaction.response.send_message("👻 Unsichtbar.", ephemeral=True)
    ...
    async def reveal(self, interaction, _):
        ...
        await interaction.response.send_message("👁 Sichtbar.", ephemeral=True)
    ...
    # 🔽 EIN Limit-Button (statt Increase + Decrease)
